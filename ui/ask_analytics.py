...
def _get_registry_tables(registry: SchemaRegistry) -> List[str]:
    reg = registry.load() or {}
    tables = sorted(list((reg.get("tables") or {}).keys()))
    return tables
...
def _suggest_tables(
    *,
    ...
    """
    Suggest tables using SchemaRegistry keys (same naming SQLAgent validates against).
    """
    from agents.planner_agent import PlannerAgent  # lazy import

    all_tables = _get_registry_tables(registry)
...
def render_ask_analytics(settings: Settings, trace_store: TraceStore, developer_mode: bool) -> None:
    ...
    # ✅ IMPORTANT: UI uses registry tables (same as SQLAgent validation)
    all_tables = _get_registry_tables(registry)
    if not all_tables:
        st.warning("Schema registry not available yet. (Auto bootstrap should run.)")
        return
